/*
 *  Этот файл содержит огрызки библиотеки работы с termcap редактора red
 *  производства ДЕМОС.
 *  Ко всем функциям добавлен префикс t_ для того, чтобы везде, где не
 *  оговорено специально, использовались стандартные функции (в последних
 *  UNIXах SVR4 работающие только через termlib)
 *  Так как приведенные здесь функции используются мной исключительно для
 *  получения описаний клавиатуры, обрезана работа с TERMCAP и используется
 *  keycapfile
 *
 *      Редактор RED.
